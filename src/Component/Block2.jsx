...
function Block2(props) {
    ...
    useEffect(() => {
        fetch(`https://newsapi.org/v2/top-headlines?category=${props.type2}&apiKey=5dd2b8e1462c4cbbb8cee6f9c828e140`).then((data) => data.json()).then((res) => setnews(res.articles))
    })
    return (
        <div>
            {news.map((data) => (
                <Link to={data.url} target='_blank' key={data.id}>
                    <div className='flex gap-1 items-start my-3 border-2 p-1' >
